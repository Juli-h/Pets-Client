...
const Signup = ({ closeSignupModal }) => {
    ...
    const message = password !== confirmPassword ? `Passwords don't match` : 'One or more fields are filled out incorrectly. Please check your entries and try again.'
    ...
    return (
        <div className="d-flex justify-content-center align-items-center mt-5">
            <form className="d-flex flex-column justify-content-center p-5 bg-white w-100" onSubmit={(event) => handleFormSubmit(event)}>
                {isMessageNotHide &&
                ...
                <div className="row my-3 d-flex justify-content-between">
                    ...
                    <div className="col-sm-10 w-75">
                        <input type="tel" className="form-control"
                            ...
                            id="phone" name="phone" placeholder="054-55662500" pattern="[0-9]{3}-[0-9]{8}" required />
                    <small className="">Format: 054-55662500 (For Your Cell Phone In Israel)</small>

                    </div>
                </div>
                <button className="btn btn-dark rounded-pill mt-2" type="submit" >Sign In</button>
            </form>
        </div>
    )
}
